...
- Shareable routes set their own SEO metadata. Home, season list, team detail, and khai hoang pages each define route-specific title, description, canonical URL, Open Graph, and Twitter metadata.
...
- Deep-linked team routes initialize the drawer open immediately. `TeamDetailDrawer` is a fixed SSR-rendered drawer shell and renders selected team content through `TeamDetailContent`, so direct links do not wait for a client-only slideover handoff.
